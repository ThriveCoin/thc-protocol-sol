...
contract ThriveReview is OwnableUpgradeable, IThriveReview {
    ...
    modifier onlyUserWithAtLeastOneBadge(bytes32[] memory badges) {
        ...
        require(
            hasAtLeastOneBadge, "User must have at least one required badge"
        );
        _;
    }
    ...
    modifier submissionPending(uint256 submissionId_) {
        require(
            idToSubmission[submissionId_].status == SubmissionStatus.PENDING,
    ...
    event SubmissionDecisionReached(
        uint256 indexed submissionId, Decision decision
    );
    ...
    event FailedDistributionFundsClaimed(
        address indexed claimer, uint256 amount
    );
    ...
    event SubmissionDisputeResolved(
        uint256 indexed submissionId, Decision decision
    );
    ...
    function initialize(
        ...
        require(
            reviewConfiguration_.agreementThreshold > 5000,
            "Agreement threshold must be above 50%"
        );
        ...
        if (reviewConfiguration.workUnit != address(0)) {
            ...
            uint256 maxSubmissionsByFundsOnWorkerUnit = IThriveWorkerUnit(
                workerUnitAddress
            ).maxRewards() / IThriveWorkerUnit(workerUnitAddress).rewardAmount();

            require(
                maxSubmissions <= maxSubmissionsByFundsOnWorkerUnit,
                "Not enough funds on worker unit contract to pay submissions"
            );
        }

        // Calculate amount of funds needed to reserve for reviewers (potentially max reviewers on submission)
        submitterReservedFunds = reviewConfiguration.reviewerReward
            * reviewConfiguration.maximumReviewsPerSubmission;

        // Emit event
        emit ThriveReviewInitialized(
            thriveReviewFactoryAddress_, badgeQueryContractAddress_, owner_
        );
    }
        ...
        returns (uint256 submissionId)
    {
        ...
        require(
            !userHasAcceptedSubmission(_msgSender()),
            "User already has an accepted submission"
        );
        ...
        require(
            msg.value >= submitterReservedFunds,
            "Insufficient funds to pay reviewers"
        );
        ...
        submission.reviewDeadline =
            uint64(block.timestamp + reviewConfiguration.reviewDeadlinePeriod);
        ...
        submissionPending(submissionId_)
    {
        ...
        require(
            idToSubmission[submissionId_].contributor == _msgSender(),
            "Caller has not submitted this submission"
        );
        ...
        require(
            submissionReviews[submissionId_].length == 0,
            "Submission has reviews"
        );
        ...
        submissionPending(submissionId_)
    {
        ...
        require(
            !userCommittedToReview[_msgSender()][submissionId_],
            "User has already committed to review this submission"
        );
        ...
        require(
            block.timestamp <= idToSubmission[submissionId_].reviewDeadline,
        ...
        review.commitmentDeadline =
            block.timestamp + reviewConfiguration.reviewCommitmentPeriod;
        ...
        onlyUserWithAtLeastOneBadge(reviewConfiguration.reviewerBadges)
    {
        ...
        Submission storage submission =
            idToSubmission[committedReview.submissionId];

        // Require user to have committed to the review
        require(
            committedReview.status == ReviewStatus.COMMITTED,
            "User has not committed to this review"
        );
        ...
        require(
            committedReview.reviewer == _msgSender(),
            "User is not the committer of this review"
        );

        // Require that the commitment deadline has not passed
        require(
            block.timestamp <= committedReview.commitmentDeadline,
            "Review commitment deadline has passed"
        );
        ...
        userInvolvedInSubmission[_msgSender()][committedReview.submissionId] =
            true;
    ...
    function deletePendingReview(uint256 reviewId_) public {
        ...
        require(
            reviews[reviewId_].status == ReviewStatus.COMMITTED,
            "Review is not in 'COMMITTED' status"
        );

        // Require that the reviews' deadline has passed
        require(
            block.timestamp > reviews[reviewId_].commitmentDeadline,
        ...
        submissionPending(submissionId_)
    {
        ...
        emit SubmissionDecisionReachedAsBadge(
            submissionId_, decision_, _msgSender()
        );
    }
    ...
    function retrieveFundsByOwner() external onlyOwner {
        ...
        uint256 leftAfterPayouts =
            address(this).balance - failedDistributionTotalAmount;

        // Transfer funds to owner
        (bool success,) =
            payable(_msgSender()).call{value: leftAfterPayouts}("");
        require(success);
     ...
     * @param submissionId_ Submission ID.
     */
    function cancelDisputeOnSubmission(uint256 submissionId_)
        external
        onlyOwner
    {
        // Fetch submission from storage
     ...
     */
    function distributePayoutsForNonDisputedSubmissions(
        uint256[] calldata submissionIds_
    ) external {
     ...
     * @param submissionId_ Submission ID.
     */
    function distributePayoutsForNonDisputedSubmission(uint256 submissionId_)
        public
    {
        // Fetch submission from storage
        Submission memory submission = idToSubmission[submissionId_];
    ...
    function _distributeRewardsForSubmission(uint256 submissionId_) internal {
        ...
        require(
            submission.status == SubmissionStatus.FINALIZED
                || submission.status == SubmissionStatus.DISPUTED,
            "Submission is not in 'FINALIZED' or 'DISPUTED' status"
        );
        ...
        for (uint256 i = 0; i < reviewIds.length; i++) {
            ...
            if (submission.decision == review.decision) {
                ...
                (bool success,) = review.reviewer.call{
                    value: reviewConfiguration.reviewerReward
                }("");
                if (!success) {
                    failedDistributionAmounts[review.reviewer] +=
                        reviewConfiguration.reviewerReward;
                    failedDistributionTotalAmount +=
                        reviewConfiguration.reviewerReward;

                    // Emit event for failed payout
                    emit ReviewerPayoutFailed(
                        submissionId_,
                        review.id,
                        review.reviewer,
                        reviewConfiguration.reviewerReward
        ...
        if (submission.decision == Decision.ACCEPTED && hasWorkerUnitContract())
        {
            ...
            IThriveWorkerUnit(workerUnitAddress).confirm(
                submission.contributor, submission.submissionMetadata
            );
        }
    }
    ...
    function _payoutSubmitterReservedFunds(uint256 submissionId_) internal {
        ...
        if (submission.decision == Decision.ACCEPTED) {
            ...
            // Those whose payouts failed will be able to claim manually.
            (bool success,) =
                submission.contributor.call{value: submitterReservedFunds}("");
            if (!success) {
                failedDistributionAmounts[submission.contributor] +=
                    submitterReservedFunds;
                failedDistributionTotalAmount += submitterReservedFunds;
            }

            emit SubmitterReservedFundsPaidOut(
                submissionId_,
            ...
            uint256 remainingAmountToPayout = reviewConfiguration.reviewerReward
                * submission.acceptedReviewsCount
                + reviewConfiguration.maximumReviewsPerSubmission
                - submission.reviewCount;
            ...
            // Those whose payouts failed will be able to claim manually.
            (bool success,) =
                submission.contributor.call{value: remainingAmountToPayout}("");
            if (!success) {
                failedDistributionAmounts[submission.contributor] +=
     ...
     */
    function userHasAcceptedSubmission(address user)
        public
        view
        returns (bool)
    {
     ...
     */
    function getSubmissionDecision(uint256 submissionId_)
        public
        view
        returns (Decision)
    {
     ...
     */
    function getSubmissionStatus(uint256 submissionId_)
        public
        view
